...
function getRandomQuery() {
  return spikeQueries[Math.floor(Math.random() * spikeQueries.length)];
}
...
export default function () {
  ...
  if (testType < 0.8) {
    ...
    const success = check(response, {
      ...
      "text search response time < 3000ms": (r) => r.timings.duration < 3000,
    });
    ...
    const formData = {
      image: http.file(testImage, "spike-test.jpg", "image/jpeg"),
      limit: "10",
    };
    ...
    const success = check(response, {
      ...
      "image search response time < 5000ms": (r) => r.timings.duration < 5000,
    });
...
export function setup() {
  console.log("Starting k6 spike tests...");
  console.log(`Base URL: ${__ENV.BASE_URL || "http://localhost:3000"}`);
  console.log(
    "This test will create sudden load spikes to test system resilience."
  );
