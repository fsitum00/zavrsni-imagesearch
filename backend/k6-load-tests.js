...
const textSearchDuration = new Trend("text_search_duration");
const imageSearchDuration = new Trend("image_search_duration");
...
export default function () {
  ...
  const textSearchTest = () => {
    ...
    const success = check(response, {
      ...
      "text search response time < 2000ms": (r) => r.timings.duration < 2000,
    });

    if (!success) {
      errorRate.add(1);
      console.log(
        `Text search failed for query "${query}": ${response.status} - ${response.body}`
      );
    }
  ...
  const imageSearchTest = () => {
    ...
    imageSearchDuration.add(duration);

    const success = check(response, {
      "image search status is 200": (r) => r.status === 200,
      "image search has success field": (r) => {
        try {
          const body = JSON.parse(r.body);
      ...
      "image search response time < 3000ms": (r) => r.timings.duration < 3000,
    });

    if (!success) {
      errorRate.add(1);
      console.log(`Image search failed: ${response.status} - ${response.body}`);
    }
  ...
  const textResponse = textSearchTest();
  sleep(1);

  const imageResponse = imageSearchTest();
  sleep(1);
